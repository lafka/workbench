...
export class DeviceMap extends React.Component {
   ...
   render() {
      ...
      return <div>
         <span style={{lineHeight: '1.42857', padding: '6px 12px', display: 'inline-block', color: '#ccc'}}>New path request in 19s</span>
...
export class ForceGraph extends React.Component {
  componentDidUpdate(prevProps, b) {
    ...
    let forceLayout = d3.forceSimulation(nodes)
      ...
      .force('link', d3.forceLink(links).id( (n) => n.address ).distance( (n) => (n || {}).influence || 0))
...
//}
  ...
  render() {
    let {nodes, links} = this.props.data;
    let {width, height} = this.props;
    let forceScale = d3.scaleLinear().domain(d3.extent(nodes, (node) => { return node.force; })).range([5, 20]);
    let influenceScale = d3.scaleLinear().domain(d3.extent(links, (link) => { return link.influence; })).range([1, 2]);
    return (
      <svg
         width={width}
         height={500}
