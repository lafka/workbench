...
//              <LinkContainer to="/user/auth/oauth"><NavItem>Oauth Authorizations</NavItem></LinkContainer>
    return (
        <Grid>
          <Row>
            <Col md={4} className="sidebar" style={{padding: '2rem 0'}}>
              <h4>Account</h4>
              <Nav>
...
let StorageWrapper = (Component) =>
   (props) => <UserStorage><Component {...props} /></UserStorage>
...
User.childRoutes = [
  {
    path: 'account',
    component: StorageWrapper(Account),
    childRoutes: Account.childRoutes
  },
  {
    path: 'auth/tokens',
    component: StorageWrapper(Tokens),
    childRoutes: Tokens.childRoutes
  },
  {
    path: 'auth/sessions',
    component: StorageWrapper(Sessions),
